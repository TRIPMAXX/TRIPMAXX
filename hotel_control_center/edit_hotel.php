<?php
	...
	if(isset($_SESSION['SESSION_DATA_HOTEL']['id']) && $_SESSION['SESSION_DATA_HOTEL']['id']!=""):		
		$return_data_arr = tools::find("all", TM_COUNTRIES, '*', "WHERE :all ORDER BY name ASC", array(":all"=>1));
		$country_data=array();
		if(!empty($return_data_arr)):
			$country_data=$return_data_arr;			
		endif;
		$ext_where=" AND (type IN ('Both', 'Hotel')) ";
		$return_data_arr = tools::find("all", TM_ATTRIBUTES, '*', "WHERE status=:status ".$ext_where." ORDER BY serial_number", array(":status"=>1));
			...
			if(tools::verify_token($white_list_array, $_POST, $verify_token)) {
				...
				if($check_flag==true):
					...
					if($save_hotel_data = tools::module_form_submission("", TM_HOTELS)):
						$find_updated_hotel = tools::find("first", TM_HOTELS, '*', "WHERE id=:id", array(":id"=>$_SESSION['SESSION_DATA_HOTEL']['id']));
						$_SESSION['SET_TYPE'] = 'success';
						$_SESSION['SET_FLASH'] = 'Hotel has been updated successfully.';
						header("location:edit_hotel");
						exit;
					else:
						$_SESSION['SET_TYPE'] = 'error';
...
<html>
<head>
	<title><?php echo(DEFAULT_PAGE_TITLE_CONTROL_CENTER_HOTEL);?>EDIT HOTEL</title>
	...
	<!--
		...
		{	
			$.ajax({
				url:"<?= DOMAIN_NAME_PATH_HOTEL."ajax_state_fetch";?>",
				type:"post",
				data:{
		...
		{  
			$.ajax({
				url:"<?= DOMAIN_NAME_PATH_HOTEL."ajax_city_fetch";?>",
				type:"post",
				data:{
					state_id:state_id
				},
		...
		function remove_img(cur, image_name, hotel_id)
		{
			...
			{
				$.ajax({
					url:"<?= DOMAIN_NAME_PATH_HOTEL."ajax_hotel_image_delete";?>",
					type:"post",
					data:{
						hotel_id:hotel_id,
										...
										<div class="form-group col-md-12">
											...
											?>
												<div style="display:inline-block;position:relative;">
													...
													<img src = "<?php echo(HOTEL_CONTROL_CENTER_IMAGE_PATH);?>delete.png" border = "0" alt = "" style="width:12px;height:18px;position:absolute;top:5px;right:10px;cursor:pointer;" onclick="remove_img($(this), '<?php echo $img_val;?>', <?php echo $hotel_data['id'];?>)"/>
												</div>
											<?php
													endif;
												endforeach;
