...
if(isset($_GET['auto_login_id']) && $_GET['auto_login_id']!="")
{
	...
	$hotel_id=str_replace(array(SECURITY_SALT, "hotel", AUTO_LOGIN_SECURITY_KEY), array("", "", ""), $auto_login_id);
	$object_control_center = new hotel_control_center();
	if($object_control_center->hotel_auto_login($hotel_id)) {
		if($_SESSION['SESSION_DATA_HOTEL']['status'] == 1) {
			if(isset($_GET['booking_id']) && $_GET['booking_id']!=""):
				tools::module_redirect(DOMAIN_NAME_PATH_HOTEL.'view_booking.php?booking_id='.$_GET['booking_id']);
			else:
				tools::module_redirect(DOMAIN_NAME_PATH_HOTEL.'bookings');
			endif;
		} else {
			unset($_SESSION['SESSION_DATA_HOTEL']);
			$_SESSION['SET_TYPE'] = 'error';
