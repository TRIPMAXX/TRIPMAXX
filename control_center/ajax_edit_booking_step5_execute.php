					...
					elseif($return_data_arr_booking['status']=="success"):
						...
						if(isset($_SESSION['step_1']['booking_type']) && $_SESSION['step_1']['booking_type']=="agent" && isset($_SESSION['step_1']['agent_name']) && $_SESSION['step_1']['agent_name']!=""):
							...
							if(!isset($return_data_arr['status'])):
								...
								//$data['msg']="Some error has been occure during execution.";
							elseif($return_data_arr['status']=="success"):
								if($return_data_arr['results']['payment_type']=="credit"):
									$email_temp_id=32;
									$cash_payment_str="";
								else:
									$email_temp_id=41;
									$cash_payment_str="Cash payment needed to be done within ".$return_data_arr['results']['pay_within_days'].($return_data_arr['results']['pay_within_days'] > 1 ? " days" : " day");
								endif;
								if($return_data_arr['results']['type']=="A"):
									...
									if(!empty($agent_email_template)):
										...
										$agent_mail_Body=str_replace(array("[FIRST_NAME]", "[LAST_NAME]", "[DETAILS_URL]", "[CASH_PAYMENT]"), array($return_data_arr['results']['first_name'], $return_data_arr['results']['last_name'], $agent_url_details, $cash_payment_str), $agent_email_template['template_body']);
								...
								elseif($return_data_arr['results']['type']=="G"):
									...
									if(!empty($gsm_email_template)):
										...
										$gsm_mail_Body=str_replace(array("[FIRST_NAME]", "[LAST_NAME]", "[DETAILS_URL]", "[CASH_PAYMENT]"), array($return_data_arr['results']['first_name'], $return_data_arr['results']['last_name'], $gsm_url_details, $cash_payment_str), $gsm_email_template['template_body']);
