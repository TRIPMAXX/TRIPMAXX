	...
	if(tools::verify_token($white_list_array, $_POST, $verify_token)) {
		...
		if(tools::module_data_exists_check("email_address = '".tools::stripcleantohtml($_POST['email_address'])."'", '', TM_DMC)) {
			...
			if($save_employee = tools::module_form_submission($uploaded_file_json_data, TM_DMC)) {
				$employee_email_template = tools::find("first", TM_EMAIL_TEMPLATES, $value='id, template_title, template_subject, template_body, status', "WHERE id=:id AND status=:status ", array(':id'=>4, ':status'=>1));
							...
							<div class="col-md-12 row">
								...
								<div class="box-body">
									...
									<div class="form-group col-md-6">
										...
										<input type="text" class="form-control validate[required, custom[email]], custom[email]"  value="<?php echo(isset($_POST['email_address']) && $_POST['email_address']!='' ? $_POST['email_address'] : "");?>" name="email_address" id="email_address" placeholder="Email Address" tabindex = "3" />
									</div>
									<div class="form-group col-md-6">
