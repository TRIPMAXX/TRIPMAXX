		...
		if($autentication_data->status=="success"):
			...
			if(isset($_POST['btn_submit'])) {
				...
				if(tools::verify_token($white_list_array, $_POST, $verify_token)) {
					...
					if($return_data_arr['status']=="success")
					{
						$tm_supplier_template = tools::find("first", TM_EMAIL_TEMPLATES, $value='id, template_title, template_subject, template_body, status', "WHERE id=:id AND status=:status ", array(':id'=>17, ':status'=>1));
