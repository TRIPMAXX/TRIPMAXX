<?php
	...
	if(isset($server_data['token']) && isset($server_data['token']['token']) && isset($server_data['token']['token_timeout']) && isset($server_data['token']['token_generation_time']) && tools::jwtTokenDecode($server_data['token']['token']) && ($server_data['token']['token_generation_time']+$server_data['token']['token_timeout']) > time()):
		if(isset($server_data['data']) && isset($server_data['data']['hotel_id']) && $server_data['data']['hotel_id']!=""):
			$booking_destination_list = tools::find("first", TM_BOOKING_HOTEL_DETAILS, 'GROUP_CONCAT(booking_destination_id) as destination_ids', "WHERE hotel_id=:hotel_id ", array(":hotel_id"=>$server_data['data']['hotel_id']));
			if(!empty($booking_destination_list) && $booking_destination_list['destination_ids']!=""):
				...
				if(!empty($booking_ids_list) && $booking_ids_list['booking_ids']!=""):				
					if(isset($server_data['data']) && isset($server_data['data']['booking_id']) && $server_data['data']['booking_id']!=""):
						...
						$booking_list = tools::find("all", TM_BOOKING_MASTERS." as b, ".TM_CURRENCIES." as cu", 'b.*,	cu.currency_code as currency_code, cu.currency_name as currency_name', "WHERE b.invoice_currency=cu.id AND b.id IN (".$booking_ids_list['booking_ids'].") ", array());
					endif;
